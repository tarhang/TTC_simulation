...
{
	/*
        increments each passenger's waiting time by one minute
        PRE: kipling != NULL
        POST: NONE
    */
    struct station* curr_station;

    // looping through stations:
    for(curr_station = kipling; curr_station != NULL; curr_station = curr_station -> next)
    {
        // looping through passengers and adding one to their waiting time by
        // just calling the previously written function in passenger.c
...
{
	/*
        adds num_arriving_passengers many passengers to a station (kipling)
        PRE: kipling != NULL
        POST: there are num_arriving_passengers passenger more passengers at
              the station (kipling)
    */
    ...
    for (curr_station = kipling; curr_station != NULL; curr_station = curr_station -> next)
    {
		to_add = num_arriving_passengers(curr_station);
		//printf("to add: %d\n", to_add);
		add_n_passengers(curr_station, to_add);
    }
}
